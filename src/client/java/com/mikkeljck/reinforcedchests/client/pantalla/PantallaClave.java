package com.mikkeljck.reinforcedchests.client.pantalla;

import com.mikkeljck.reinforcedchests.block.CofrePersonalBlockEntity;
import com.mikkeljck.reinforcedchests.red.RedCofres;
...
public class PantallaClave extends BaseOwoScreen<FlowLayout> {
	...
	public PantallaClave(final BlockPos pos, final String duenno) {
		super(Component.translatable("screen.reinforced_chests.titulo_clave"));
		this.pos = pos;
		this.duenno = duenno;
	}
	...
	protected void build(final FlowLayout raiz) {
		...
		raiz.child(UIContainers.verticalFlow(Sizing.content(), Sizing.content())
				.child(UIComponents.label(Component.translatable("screen.reinforced_chests.cofre_de", this.duenno))
						.shadow(true))
				.child(UIComponents.label(Component.translatable("screen.reinforced_chests.escribe_clave")))
				.child(this.campo)
				.child(UIComponents.button(
						Component.translatable("screen.reinforced_chests.abrir"),
						boton -> this.enviar()))
				.gap(8)
				.horizontalAlignment(HorizontalAlignment.CENTER)
